...
//			bindhotkey();
	...
	b(function() {
		...
		b(".button-login").on("click", function() {
			a.WBActivity.showLoading();
			console.log(PATH_ACTIVITY);
			b.getJSON(PATH_ACTIVITY + Path_url('login.do.php'), {
				rid: scene_id,
				password: b("#password").val()
			}, function(e) {
				if (e.errno==0) {
		...
		b(".mp_account_codeimage").on("click", function() {
			var isbig=$(this).hasClass('bigqrcode');
			if(isbig){
				b(".bigmpcodebar").slideDown();
				$(this).removeClass('bigqrcode');
			}else{
				$(this).addClass('bigqrcode');
			}
		});
		b(".bigmpcodebar .closebutton").on("click", function() {
			b(".bigmpcodebar").slideUp()
