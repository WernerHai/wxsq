...
function init() {
	oss = []
	targets = { table: [], torus: [], sphere: [], helix: [], grid: [] };
...
//	console.log(table);
	...
	for ( var i = 0; i < table.length; i ++ ) {
		...
		img.setAttribute('name', table[ i ].nickname);
...
//	console.log('runanimate:'+runanimate);
	//if(runanimate==0){
	runninganimate=window.requestAnimationFrame( animate );
		TWEEN.update();
		runanimate++;
	//}
...
function resetanimate(){
	window.cancelAnimationFrame(runninganimate);
}
